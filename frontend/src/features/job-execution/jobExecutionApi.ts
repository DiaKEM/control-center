  ...
  endpoints: (builder) => ({
    getJobExecutions: builder.query<JobExecution[], JobExecutionFilters>({
      query: (filters) => {
        ...
        if (filters.needsNotification)
          params.set('needsNotification', filters.needsNotification);
    ...
    deleteFilteredJobExecutions: builder.mutation<
      { deletedCount: number },
      JobExecutionFilters
    >({
      query: (filters) => {
        ...
        if (filters.to) params.set('to', filters.to);
        return {
          url: `/job-executions?${params.toString()}`,
          method: 'DELETE',
        };
      },
      invalidatesTags: ['JobExecution'],
